...
class BinaryAttention(nn.Module):
    '''
        Attention Mask.
    '''
    def __init__(self, in_channels, out_channels, fc_reduction=4, eps=0.66667, **kwargs):
        super(BinaryAttention, self).__init__()
        # Parameter
        self.bottleneck = in_channels // fc_reduction 
        self.in_channels, self.out_channels = in_channels, out_channels
        self.eleNum_c = torch.Tensor([out_channels])
        # channel attention
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.atten_c = nn.Sequential(
            nn.Conv2d(in_channels, self.bottleneck, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(self.bottleneck),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.bottleneck, out_channels, kernel_size=1, stride=1, bias=False),
        )
