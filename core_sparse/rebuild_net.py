...
from core_sparse.layers.spa_block import SparseMaxPool
from core_sparse.layers.prune_block import PaSAct, DPaSAct
...
def insert_pas_modules(network, inplace=False):
    ...
    for module_name, module in network.named_modules():
        ...
        # if isinstance(module, nn.MaxPool2d): # only star for max pool
        #     # Replace ReLU with PaSAct
        #     new_module = PaSAct(module, in_chs = in_ch_list[-1])
        #     _swap_module(module_name=module_name, network=network, new_module=new_module)
        #     print(f"""\t - \"{module_name}\": {module.__class__.__name__} -> {new_module.__class__.__name__}""")
        ...
        if isinstance(module, nn.MaxPool2d): # only star for max pool
            # Replace MaxPool with SparseMaxPool
            new_module = SparseMaxPool(module, threshold=2**-25, inplace=inplace, position="after")
            _swap_module(module_name=module_name, network=network, new_module=new_module)
            print(f"""\t - \"{module_name}\": {module.__class__.__name__} -> {new_module.__class__.__name__}""")
...
def insert_dyn_pas_modules(network, inplace=False):
    """
    Recursively traverse the network and replace Conv2d layers with SparseConv2d layers.
    """
    in_ch_list = []
    for module_name, module in network.named_modules():
        ...
        if isinstance(module, nn.ReLU):
            # Replace ReLU with DPaSAct
            new_module = DPaSAct(module, in_chs = in_ch_list[-1])
            _swap_module(module_name=module_name, network=network, new_module=new_module)
            print(f"""\t - \"{module_name}\": {module.__class__.__name__} -> {new_module.__class__.__name__}""")
