...
from torch_spatial.layers.spa_layer import Input_Threshold
...
def calculate_macs(net):
    ...
    for name, module in net.named_modules():
        if isinstance(module, Input_Threshold):
...
def measure_network_compute(net, arch, avg_metric_dit, xlsx_file_path=None, verbose=False):
    ...
    for name, m in net.named_modules():
        ...
        if isinstance(m, (nn.Conv2d, nn.Linear, nn.ConvTranspose2d, nn.LayerNorm)):
    ...
    if arch == "fuse":
        _mac_dist_table = PrettyTable()
        _mac_dist_table.field_names = [
            "name", "sigma macs", "dense macs", "density", "protion"
        ]
        _mac_dist_table.add_row(
            [
                "expansion", 
